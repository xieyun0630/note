...
### python注释 [	](python_20201224023254438)
...
### for循环与`range` [	](python_20201224023254451)
...
### while循环 [	](python_20201224023254454)
...
### 包 
+ 概念：**包含多个模块**以及`__init__.py`文件的 **特殊目录**
+ 作用: {{c1:: 使用 `import 包名` 可以一次性导入 **包** 中 **所有的模块** }}
+ `__init__.py`
  + 作用：要在外界使用 **包** 中的模块，需要在 `__init__.py` 中指定 **对外界提供的模块列表**
    ```python
    #{{c1::
    # 从当前目录导入模块列表
...
### 函数：定义 [	](python_20201224023254461)
+ 案例：求阶乘函数
	```python
    #{{c1::
	def fac(num):
...
### 函数的参数 [	](python_20201224023254464)
...
### 变量的作用域`global` `nonlocal` [	](python_20201224023254469)
...
### 字符串常见函数 [	](python_20201224023254481)
+ 串：`str1 = 'hello, world!'`
+ 常见操作：
...
+ 查找子串所在位置 :{{c1:: `print(str1.find('or')) # 8 :{ }}{c1:: `print(str1.find('shit')) # -1` }}
...
### 字符串：格式化输出 [	](python_20201224023254484)
+ 变量声明：`a, b = 5, 10`
+ 几种格式输出
...
### 列表：运算操作 [	](python_20201224023254488)
...
### 列表：遍历 [	](python_20201224023254491)
...
### 列表：增删查改 [	](python_20201224023254494)
...
+ 清空列表元素
	...
	#}}
	```
### 列表：切片操作 [	](python_20201224023254496)
...
### 列表：排序 [	](python_20201224023254498)
...
### 列表：生成式和生成器 [	](python_20201224023254500)
...
+ 创建和使用对象:
  ...
  ```
### python类的语法细节
+ 查询方法的内置函数：{{c1:: `dir()`,传入 **标识符** / **数据**，可以查看对象内的 **所有属性及方法** }}
+ 常用的内置方法
  | 序号 |   方法名   | 类型 | 作用                                         |
  | :--: | :--------: | :--: | -------------------------------------------- |
