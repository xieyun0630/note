...
### MYSQL基本操作 [ ](mysql_20200824100449026)r [	](mysql_20201224024146013)
...
### mysql显示宽度 [ ](mysql_20200911094545396)
...
### mysql字符串类型（文本字符串） [ ](mysql_20200911094545413)
...
+ 作用：{{c1:: 查看服务器生成的二进制日志文件 }}
  + 语法：{{c1:: `mysqlbinlog [options]  log-files1 log-files2 ...` }}
  + 选项：
...
+ 作用：{{c1:: 记录了所有的 DDL（数据定义语言）语句和 DML（数据操纵语言）语句，但是不包括**数据查询语句**，主从复制，就是通过该binlog实现，默认情况下是没有开启的 }}
+ 配置文件位置 : {{c1:: /usr/my.cnf }}
  + 开启日志：{{c1:: `log_bin=mysqlbin` }}
  + 配置日志格式：{{c1:: `binlog_format=STATEMENT` }}
+ 日志格式
  + **STATEMENT**:{{c1:: 记录的都是SQL语句 }}
  + **ROW**:{{c1:: 记录的是每一行的数据变更 }}
  + **MIXED**:{{c1:: 混合了STATEMENT 和 ROW两种格式,默认情况下采用STATEMENT，但是在一些特殊情况下采用ROW来进行记录。 }}
+ 查看日志:{{c1:: `mysqlbinlog log-file；` }}
+ 查看ROW格式日志:{{c1:: `mysqlbinlog -vv mysqlbin.000002` }}
