...
1. **抽象部分**: {{c1:: （Abstraction）提供高层控制逻辑，依赖于完成底层实际工作的实现对象。 }}
2. **实现部分**: {{c1:: （Implementation）为所有具体实现声明通用接口。抽象部分仅能通过在这里声明的方法与实现对象交互。 }}
3. **具体实现**: {{c1:: （ConcreteImplementations）中包括特定于平台的代码。 }}
4. **精确抽象**: {{c1:: （RefinedAbstraction）提供控制逻辑的变体。与其父类一样，它们通过通用实现接口与不同的实现进行交互。 }}
5. **客户端**: {{c1:: （Client）仅关心如何与抽象部分合作。但是，客户端需要将抽象对象与一个实现对象连接起来。 }}
