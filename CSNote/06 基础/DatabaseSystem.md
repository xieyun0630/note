...
+ 什么是锁: ![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210630111626.png)![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210630111707.png)
+ 封锁协议常见锁类型：
...
+ 封锁协议之相容性矩阵：![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210630112148.png)
+ 封锁协议需要考虑4个方面：
  1. {{c1::封锁的类型}}
  2. {{c1::封锁的粒度}}
  3. {{c1::相容性矩阵}}
...
+ 幻读的概念：{{c1::指的是事务不是串行发生时的一种现象，是事务A读取了事务B已提交的新增数据。例如第一个事务对一个表的所有数据进行修改，同时第二个事务向表中插入一条新数据。那么操作第一个事务的用户就发现表中还有没有修改的数据行，就像发生了幻觉一样。解决幻读的方法是增加范围锁（range lock）或者表锁。}}
...
### 基于时间戳的并发控制方法 [ ](DatabaseSystem_20210702082554365)
+ 定义：{{c1::借助于时间戳，强制使一组并发事务的交叉执行，等价于一个特定顺序的串行执行}}
+ 图示：{{c1::![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210630132038.png)}}

### 基于有效性确认的并发控制的思想 [ ](DatabaseSystem_20210702082554367)
+ 图示:{{c1::![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210630132259.png)}}

## 故障恢复 [ ](DatabaseSystem_20210702082554369)

### 数据库的故障类型及其影响 [ ](DatabaseSystem_20210702082554371)
+ 数据库故障类型
  + **事务故障**：{{c1::某一个程序(事务)自身运行错误所引起的故障，**影响**该程序(事务)本身}}
  + **系统故障**：{{c1::由于掉电、非正常关机等所引起的故障，影响正在运行的事务以及数据库缓冲区, 数据库缓冲区将涉及正在运行和已经运行的事务}}
  + **介质故障**：{{c1::由于介质损坏等所引起的故障，影响是全面的，既影响内存中的数据, 又影响介质中存储的数据}}
+ 数据库数据存储3种层次图：{{c1::![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210701152854.png)}}

### 数据库故障恢复 [ ](DatabaseSystem_20210702082554373)
+ 核心作用：{{c1::把DB由当前不正确状态恢复到**已知为正确**的某一状态}}
+ 需要保证事务的：
  + {{c1:: 原子性，事务的所有操作，要么全都执行，要么全都不执行。}}
  + {{c1:: 持久性，已提交的事务对数据库产生的影响是持久的，未提交的事务对数据库不应有影响。}}
+ 事务故障的恢复
  + Redo方式：{{c1::重做事务(Redo)可**保证**已提交事务的**持久性**}}
  + Undo方式：{{c1::撤销事务则**消除未提交事务**的影响}}

### 系统故障恢复 [ ](DatabaseSystem_20210702082554375)
+ 运行日志
    + 定义：{{c1::System Log，运行日志是DBMS维护的一个文件，该文件以**流水方式**记录了每一个事务对数据库的**每一次操作及操作顺序**，运行日志直接写入介质存储上，会保持正确性}}
    + 作用：{{c1::系统故障可通过运行日志来恢复，按照运行日志记录的事务操作顺序**重做事务**(当事务在发生故障时已正确结束)或**撤消事务**(当事务在发生故障时未结束) }}
    + 当事务对数据库进行操作时日志的位置（图示）：{{c1::先写运行日志1；写成功后，再与数据库缓冲区进行信息交换2；![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210701160654.png)}}
+ 运行日志中检查点概念：
    + 定义：{{c1::(checkpoint)，检查点是这样的时刻: 在该时刻, DBMS强制使内存DB Buffer中的内容**与介质DB中的内容保持一致**，即将DB Buffer更新的所有内容写回DB中。检查点表征了：在检查点之前内存中数据与介质中数据是保持一致的}}
    + 作用：{{c1::检查点之后结束或发生的事务需要依据运行日志进行恢复(不能确定是否写回DB): 故障点前结束的**重做**,故障点时刻未结束的**撤消**![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210701162502.png)}}

### 介质故障恢复 [ ](DatabaseSystem_20210702082554377)
+ 副本(Copy)：{{c1::在某一时刻，对数据库在其他介质存储上产生的另一份等同记录，用副本替换被损坏的数据库![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210701164140.png)}}
+ 介质故障的恢复：{{c1::用副本替换被破坏的数据库，由于介质故障影响全面, 在**用副本恢复后还需要依据运行日志进行恢复**}}
+ 转储点：
  + 图示：{{c1::![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210701164343.png)}}
  + 生成转储点时机：{{c1::**过频**，影响系统工作效率；**过疏**，会造成运行日志过大，也影响系统运行性能，备份转储周期与运行日志的大小密切相关，应注意**防止衔接不畅**而引起的漏洞}}

### 数据库故障恢复的宏观思路 [ ](DatabaseSystem_20210702082554379)
+ 三种类型故障：{{c1::**事务故障**、**系统故障**和**介质故障**}}
+ 三种恢复手段: {{c1::事务的**撤消与重做**, **运行日志**和**备份**}}
+ 两个重要时刻：{{c1::**检查点**和**转储点**}}
+ 图示：{{c1::![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210701164843.png)}}

### 事务涉及到的 [ ](DatabaseSystem_20210702082554381)
+ 数据库通常由元素构成：{{c1::![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210701165552.png)}}
+ 每个事务都会读/写某些元素，四种操作：{{c1::![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210701165806.png)}}
+ 每个事务都以提交或者撤销结束
  + {{c1::**COMMIT**：事务提交}}
  + {{c1::**ABORT**：事务撤销}}
