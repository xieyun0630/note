...
### 排序：基本概念 [ ](dataStructure_20210206010825327)
...
## 树与森林 [ ](dataStructure_20210122105026672)
...
+ 复杂度分析：
  ...
  + 时间复杂度：{{c1:: 邻接矩阵`O(|V|<sub>2</sub>)`,邻接表`O(|V|+|E|)`}}
+ 深度优先遍历序列:{{c1:: 同⼀个图的邻接矩阵表示⽅式唯⼀，因此深度优先遍历序列唯⼀,同⼀个图邻接表表示⽅式不唯⼀，因此深度优先遍历序列不唯⼀}}
...
+ **最小生成树具有如下性质**
  1. 最小生成树不是唯一的：{{c1:: 即最小生成树的树形不唯一，究中可能有多个最小生成树。当图G中的各边权值互不相等时，G的最小生成树是唯一的；若无向连通图G的边数顶点数少1,即G本身是一棵树时，则G的最小生成树就是它本身。}}
...
+ 矩阵计算关键步骤
  ...
  2. {{c1:: 则 `A(k)[i][j] = A(k−1)[i][k]+ A(k−1)[k][j]；` `path(k)[i][j] = k` }}
...
| 带负权值的图   | {{c1::❌}}                                     | {{c1::❌}}                       | {{c1::✔}}                             |
| 带负权回路的图 | {{c1::❌}}                                     | {{c1::❌}}                       | {{c1::❌}}                             |
| 时间复杂度     | {{c1:: O(\|V\|<sup>2</sup>)或O(\|V\|+\|E\|)}} | {{c1:: O(\|V\|<sup>2</sup>)}}   | {{c1:: O(\|V\|<sup>3</sup>)}}         |
...
+ AOE⽹具有以下两个性质：
  ...
  2. {{c1:: 只有在进⼊某顶点的各有向边所代表的活动都已结束时，该顶点所代表的事件才能发⽣。另外，有些活动是可以并⾏进⾏的 }}
+ **开始顶点**：{{c1:: 在AOE⽹中仅有⼀个⼊度为0的顶点，称为开始顶点（源点），它表示整个⼯程的开始； }}
...
- 5大步骤：{{c1::![](https://gitee.com/xieyun714/nodeimage/raw/master/img/20210218014953.png)}}
- 例：{{c1::![image-20210218015142855](https://gitee.com/xieyun714/nodeimage/raw/master/img/image-20210218015142855.png)}}
+ 关键路径与关键活动特性：
  + 若关键活动耗时增加，{{c1:: 则整个工程的工期将增长 }}
  + 缩短关键活动的时间，{{c1:: 可以缩短整个工程的工期 }}
...
+ **静态查找表**:只用于查找的表。仅关注查找速度即可
+ **动态查找表**:除了用于查找也**需要插入与删除**。除了查找速度，也要关注插/删操作是否⽅便实现。
...
+ 插入算法思想：
  + 查找插⼊位置：{{c1::新元素⼀定是插⼊到最底层“终端节点”，⽤“查找”来确定插⼊位置}}
  + 超过上限分裂过程：{{c1::在插⼊key后，若导致原结点关键字数超过上限，则从中间位置`ceil(m/2)`将其中的关键字**分为两部分**，左部分包含的关键字放在原结点中，右部分包含的关键字放到新结点中，中间位置`ceil(m/2)`的结点插⼊原结点的⽗结点。若此时导致其⽗结点的关键字**个数**也超过了**上限**，则**继续**进⾏这种**分裂**操作，直⾄这个过程传到根结点为⽌，进⽽导致B树⾼度增1。}}
