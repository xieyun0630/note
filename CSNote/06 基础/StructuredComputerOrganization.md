...
+ 各步骤解释：
    #1：{{c1::(PC)->MAR，导致(MAR)=1}}
    #3：{{c1::M(MAR)->MDR，导致(MDR)=000100 0000000110}}
    #4：{{c1::(MDR)->IR，导致(IR)= 000100 0000000110}}
    #5：{{c1::OP(IR)->CU，指令的操作码送到CU， CU分析后得知，这是“乘法”指令}}
    #6：{{c1::Ad(IR)->MAR，指令的地址码送到MAR，导致(MAR)=6}}
    #8：{{c1::M(MAR)->MDR，导致(MDR)=0000000000000011=3}}
    #9：{{c1::(MDR)->MQ，导致(MQ)=0000000000000011=3}}
    #10：{{c1::(ACC)->X，导致(X)=2}}
    #11：{{c1::(MQ)*(X)->ACC，由ALU实现乘法运算，导致(ACC)=6，如果乘积太大，则需要MQ辅助存储}}
